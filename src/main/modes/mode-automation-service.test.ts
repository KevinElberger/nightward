...
const createModeAutomationService = ({
  ...
  const actionRunner = {
    runActions: vi.fn().mockResolvedValue(undefined)
...
describe('ModeAutomationService', () => {
  it('runs enter actions when a mode activates', async () => {
    ...
    await expect(automationService.activateMode('focus-mode')).resolves.toBe(true);
  ...
  it('runs exit actions when a mode deactivates', async () => {
    ...
    await expect(automationService.deactivateMode()).resolves.toBe(true);
  ...
  it('does not run actions when mode state does not change', async () => {
    ...
    await expect(automationService.activateMode('focus-mode')).resolves.toBe(false);
    await expect(automationService.deactivateMode()).resolves.toBe(false);
